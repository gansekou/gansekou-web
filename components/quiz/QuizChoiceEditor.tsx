...
export function QuizChoiceEditor({
  choices,
  questionType,
  labels,
  onChange,
  ...
  function setChoice(
    index: number,
    patch: Partial<QuizChoiceCreatePayload>
  ) {
  ...
  const normalizedChoices =
    ...
    choices.length < 2
      ? [
          choices[0] || {
            choice_text: labels.trueLabel,
            is_correct: true,
          },
          choices[1] || {
            choice_text: labels.falseLabel,
            is_correct: false,
          },
        ]
      : choices;
      ...
      {normalizedChoices.map((choice, index) => {
        ...
        return (
          <div
            key={
              choice.id ||
              `quiz-choice-editor-${questionType}-${index}`
            }
            className="
